	...
	<div class="contents-box">
		...
		<c:forEach items="${cardList}" var="card">
			...
			<div class="card border rounded mt-3">
				...
				<c:forEach items="${card.comment}" var="commentView">
					<div class="card-comment m-1"><!-- F3 누르면 안쪽으로 들어감 -->
					...
					<div class="comment-write d-flex border-top mt-2">
						<input type="text" id="commentContent" class="form-control border-0 mr-2" placeholder="댓글 달기"/> 
...
<script>
	$(document).ready(function() {
		...
		$('.more-btn').on('click', function(e) {
			e.preventDefault(); // a 태그에서는 거의 항상
			...
			$.ajax({
				...
				, success: function(data) {
					if (data.code == 1) {
						...
						alert("게시물을 삭제하지 못했습니다.1");
					}
				}
				, error: function(e) {
					alert("게시물을 삭제하지 못했습니다.2");
				}
			});
		});
	});
...
<!-- - 댓글 API Controller, BO, DAO, mapper

- view 댓글 내용 

회원가입, 글쓰기, 로그인, 글 목록, 댓글 쓰기 + 댓글 목록 뿌리기 -->

<!-- List<Post> => List<CardView>

JSP -> Controller -> timelineBO / -> PostBO(/CommentBO/LikeBO) -> PostDAO

좋아요 누르기 API(되거나 해제되거나) => DB 추가/삭제

타임라인에 내가 좋아요를 했는지 여부 -->